...
|:--|:--|:--|
|`*`|*name|指向通用图素|
|`$`|$name|该控件对外暴露，要绑定程序逻辑。注意，只有控件可以对外导出，而属性层不可以，比如placeholder，text，sprite等。|
|`!`|!name|忽略该层（组），不对外导出|
...
|控件|描述|规则
|--:|:--|:--|
|bound|限定控件大小|图片层，需明确指出=bound，该层不会导出，但是bound会覆盖到控件的的bound上，用来设定控件的offset和size，包括内含的所有`文本属性层`。|
